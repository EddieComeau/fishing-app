const { getSessionStart } = require('../services/sessionStart.service');
...
async function getSessionStartHandler(req, res) {
  try {
    ...
    const result = await getSessionStart({
      lat,
      lng,
      waterType,
      accessMode,
    ...
    return res.status(status).json({ error: error.message || 'Failed to build session-start guidance' });
  }
}
